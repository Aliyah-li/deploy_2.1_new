/**
 * @file strong2_rl_control_state.hpp
 * @brief RL control state for Strong2 — variable-height (变高度), 58-dim obs
 *
 * Uses M20Fft2PolicyRunner (same 变高度 engine as FFT2).
 * Default policy: strong_ppo_base/policy.onnx
 * Initial height = 0.38 m (matches StandUp stand_height_).
 *
 * Config reference: sim2sim_deploy_config.md
 * Observation (58 dims):
 *   [0:3]   base_ang_vel * 0.25
 *   [3:6]   projected_gravity
 *   [6:9]   velocity commands: vx, vy, yaw command
 *   [9:25]  joint_pos_rel (16, wheel positions zeroed)
 *   [25:41] joint_vel * 0.05 (16)
 *   [41:57] last_action (16)
 *   [57]    base_height_command (1, appended last by training env)
 */
...
namespace qw {
    class Strong2RLControlState : public StateBase {
    private:
        RobotBasicState rbs_;
        int state_run_cnt_;

        std::shared_ptr<PolicyRunnerBase> policy_ptr_;
        std::shared_ptr<M20Fft2PolicyRunner> strong2_policy_;
        ...
        void PolicyRunner() {
            ...
            std::cerr << "[STRONG2] PolicyRunner thread started" << std::endl;
            while (start_flag_) {
                try {
                if (state_run_cnt_ % policy_ptr_->decimation_ == 0 && state_run_cnt_ != run_cnt_record) {
                    ...
                    std::cerr << "[STRONG2] PolicyRunner exception: " << e.what() << std::endl;
                } catch (...) {
                    std::cerr << "[STRONG2] PolicyRunner unknown exception" << std::endl;
                }
                std::this_thread::sleep_for(std::chrono::microseconds(10));
            }
            std::cerr << "[STRONG2] PolicyRunner thread exiting" << std::endl;
        }

    public:
        Strong2RLControlState(const RobotName &robot_name, const std::string &state_name,
            ...
            if (robot_name_ == RobotName::M20) {
                ...
                if (!fs::exists(policy_base)) {
                    ...
                    if (fs::exists(src_base)) {
                        ...
                        std::cerr << "[STRONG2 ERROR] Cannot find policy directory at:\n"
                ...
                const char* env_subdir = std::getenv("STRONG2_POLICY_SUBDIR");
                std::string subdir = env_subdir ? env_subdir : "strong_ppo_base";
                auto strong2_policy_path = policy_base / subdir / "policy.onnx";

                if (!fs::exists(strong2_policy_path)) {
                    std::cerr << "[STRONG2 ERROR] Policy not found: "
                              << strong2_policy_path.string() << "\n";
                    exit(1);
                }

                strong2_policy_ = std::make_shared<M20Fft2PolicyRunner>(
                    "strong2_policy", strong2_policy_path.string(), false);
            }

            policy_ptr_ = strong2_policy_;
            if (!policy_ptr_) {
                std::cerr << "[STRONG2 ERROR] Failed to create Strong2 policy runner" << std::endl;
                exit(0);
            }
        ...
        ~Strong2RLControlState() {}

        virtual void OnEnter() {
            std::cerr << "[STRONG2] OnEnter RLControl START" << std::endl;
            state_run_cnt_ = -1;
            start_flag_ = true;
            auto* cmd = uc_ptr_->GetUserCommand();
            if (!cmd) {
                std::cerr << "[STRONG2] FATAL: GetUserCommand returned null" << std::endl;
                exit(1);
            }
            ...
            std::cerr << "[STRONG2] OnEnter starting policy thread..." << std::endl;
            try {
                run_policy_thread_ = std::thread(std::bind(&Strong2RLControlState::PolicyRunner, this));
            } catch (const std::exception& e) {
                std::cerr << "[STRONG2] Exception starting policy thread: " << e.what() << std::endl;
                exit(1);
            }
            std::cerr << "[STRONG2] OnEnter calling policy_ptr_->OnEnter..." << std::endl;
            policy_ptr_->OnEnter(rbs_);
            StateBase::msfb_.UpdateCurrentState(RobotMotionState::RLControlMode);
            std::cerr << "[STRONG2] OnEnter RLControl DONE" << std::endl;
        };
        ...
        virtual void Run() {
            ...
            if (state_run_cnt_ % 50 == 0) {
                float h = strong2_policy_->GetHeightTarget();
                std::cout << "[STRONG2] omega=" << rbs_.base_omega.transpose()
                          << " height=" << h << "\n";
            }
            state_run_cnt_++;
        }
