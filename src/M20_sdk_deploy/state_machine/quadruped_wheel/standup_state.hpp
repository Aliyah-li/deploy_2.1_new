...
private:
    ...
    void OpenCsv(){
        mkdir("logs", 0755);
        csv_file_.open("logs/standup_height_joint_data.csv", std::ios::out | std::ios::trunc);
        if (!csv_file_.is_open()) {
            std::cerr << "[StandUp] Cannot open CSV output" << std::endl;
            return;
        }
        csv_file_ << "time_s,phase,command_height_m,base_height_m";
        for (int i = 0; i < 16; ++i) csv_file_ << ",joint_" << i << "_pos";
        for (int i = 0; i < 16; ++i) csv_file_ << ",joint_" << i << "_target";
        csv_file_ << '\n';
        csv_file_ << std::fixed << std::setprecision(8);
    }

    void WriteCsv(float elapsed, const char* phase, float command_height,
                  const VecXf& joint_target){
        if (!csv_file_.is_open()) return;
        csv_file_ << elapsed << ',' << phase << ',' << command_height << ','
                  << ri_ptr_->GetBasePosition().z();
        for (int i = 0; i < current_joint_pos_.rows(); ++i) csv_file_ << ',' << current_joint_pos_(i);
        for (int i = 0; i < joint_target.rows(); ++i) csv_file_ << ',' << joint_target(i);
        csv_file_ << '\n';
        if (elapsed >= 2.0f * static_cast<float>(stand_duration_)) {
            csv_file_.close();
            std::cout << "[StandUp] CSV saved: logs/standup_height_joint_data.csv" << std::endl;
        }
    }
...
public:
    ...
    virtual void Run() {
        ...
        float command_height = cp_ptr_->pre_height_;
        const char* phase = "joint_to_pre_height";
        ...
        joint_cmd_.col(2) = kd_;
        WriteCsv(run_time_ - time_stamp_record_, phase, command_height, planning_joint_pos);
