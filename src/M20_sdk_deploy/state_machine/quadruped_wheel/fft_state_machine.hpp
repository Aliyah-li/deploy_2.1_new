/**
 * @file fft_state_machine.hpp
 * @brief State machine for FFT (Fast Fine-Tuning) sim2sim deployment
 * @author DeepRobotics
 * @version 1.0
 * @date 2025-11-07
 *
 ...
 *
 * State transitions:
 *   kIdle → kStandUp → kRLControl (FFT policy)
 *   any → kJointDamping (on safety trigger)
 *
 * Differences from QwStateMachine:
 *  - Uses FftRLControlState (M20FftPolicyRunner) instead of RLControlState
 *  - Single policy (no speed-mode / direction switching overhead)
 */
...
#include "quadruped_wheel/fft_rl_control_state.hpp"
...
namespace qw {
class FftStateMachine : public StateMachineBase {
private:
...
public:
    ...
    FftStateMachine(RobotName robot_name, RemoteCommandType rct)
    ...
    void Start() {
        ...
        rl_controller_            = std::make_shared<FftRLControlState>(robot_name_, "fft_rl_control", data_ptr);
        ...
        uc_ptr_->Start();   // keyboard first — must init before ri_ptr_ blocks
        ri_ptr_->Start();
        sc_ptr_->Start();
        current_controller_->OnEnter();

        std::cout << "[FFT] State machine started. Policy base: M20FftPolicyRunner" << std::endl;
    }
    ...
    std::shared_ptr<StateBase> GetStateControllerPtr(StateName state_name) {
        switch (state_name) {
            ...
            default: {
                std::cerr << "[FFT] error: unknown state name" << std::endl;
