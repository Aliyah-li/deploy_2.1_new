...
public:
    ...
    /**
     * @brief Get base linear velocity in the body frame.
     *
     * Hardware backends without a velocity estimator keep the default zero
     * value.  The MuJoCo bridge publishes the exact simulated velocity.
     */
    virtual Vec3f GetBaseLinearVelocity() {
        return Vec3f::Zero();
    }

    /** Get simulated base position in world coordinates when available. */
    virtual Vec3f GetBasePosition() {
        return Vec3f::Zero();
    }
