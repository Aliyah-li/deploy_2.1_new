...
# ============================================================
# fixed_height_simulation/flat_simulation.sh — fixed-height flat PPO Sim2Sim
# ============================================================
#
# Launches:
...
#
# Usage:
#   ./fixed_height_simulation/flat_simulation.sh [x86|aarch64]
#   RECORD_QPOS_PATH=/tmp/qpos.npz ./fixed_height_simulation/flat_simulation.sh
# ============================================================
...
if command -v ros2 >/dev/null 2>&1; then
  ...
  gnome-terminal --tab --title="RL Deploy (Strong PPO Base)" -- bash -lc "
    cd '$PWD';
    export ROS_DOMAIN_ID=$ROS_DOMAIN_ID;
    export LD_LIBRARY_PATH='$ONNXRUNTIME_LIB':\${LD_LIBRARY_PATH:-};
    export POLICY_SUBDIR=fixed_height_flat;
    $ROS_ENV_SETUP;
    source install/setup.bash;
  ...
  echo "Launched RL Deploy (Strong PPO Base) + MuJoCo Simulation."
  echo ""
  echo "┌──────────────────────────────────────────────────┐"
  echo "│ 键盘敲在 'RL Deploy (Strong PPO Base)' 终端里！  │"
  ...
  echo ""
  echo "Policy: src/M20_sdk_deploy/policy/fixed_height_flat/policy.onnx"
  echo ""
  echo "To record qpos:"
  echo "  RECORD_QPOS_PATH=/tmp/qpos.npz ./fixed_height_simulation/flat_simulation.sh"
else
