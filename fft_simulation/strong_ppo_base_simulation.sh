...
if command -v ros2 >/dev/null 2>&1; then
  ...
  gnome-terminal --tab --title="RL Deploy (Strong PPO Base)" -- bash -lc "
    ...
    export LD_LIBRARY_PATH='$ONNXRUNTIME_LIB':\${LD_LIBRARY_PATH:-};
    export POLICY_SUBDIR=strong_ppo_base;
    $ROS_ENV_SETUP;
    source install/setup.bash;
    ros2 run m20_sdk_deploy rl_deploy_fft 2>/tmp/fft_debug.log;
  ...
  echo ""
  echo "Policy: strong_ppo_base (1 input, no history, no residual)"
  echo ""
  echo "To record qpos:"
  echo "  RECORD_QPOS_PATH=/tmp/qpos.npz ./fft_simulation/strong_ppo_base_simulation.sh"
else
